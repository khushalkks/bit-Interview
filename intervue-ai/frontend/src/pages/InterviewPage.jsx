...
export default function InterviewPage() {
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-[#090d16] flex flex-col items-center justify-center text-slate-300 p-6 space-y-4">
  ...
  return (
    <div className="min-h-screen bg-[#090d16] text-slate-100 flex flex-col w-full font-sans overflow-x-hidden">
      <DashboardNavbar />
      ...
      <div className="flex-grow w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col space-y-6">
        ...
        {/* Live Session Header */}
        <div className="p-4 sm:p-6 rounded-2xl bg-slate-900/80 border border-slate-800/90 shadow-xl flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono px-2.5 py-0.5 rounded-md bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
                  Role: {session.target_role}
                </span>
              </div>
              <h1 className="text-lg sm:text-xl font-black text-white mt-1 flex items-center gap-2">
                Adaptive AI Room <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-slate-800/80 border border-slate-700/60 text-xs font-semibold">
              <Zap className="w-4 h-4 text-amber-400 animate-pulse" />
              <span className="text-slate-400">Difficulty:</span>
              <span className={`font-mono ${
                session.current_difficulty === 'Advanced' ? 'text-rose-400' :
                session.current_difficulty === 'Hard' ? 'text-purple-400' :
                session.current_difficulty === 'Medium' ? 'text-cyan-400' : 'text-emerald-400'
              }`}>
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-rose-600 to-amber-600 hover:from-rose-700 hover:to-amber-700 text-white font-bold text-xs sm:text-sm shadow-lg shadow-rose-500/20 flex items-center gap-2 disabled:opacity-50 transition-all hover:scale-[1.02]"
          >
            {ending ? (
              <>
        ...
        <div className="flex-grow min-h-[420px] max-h-[560px] overflow-y-auto p-6 rounded-3xl bg-slate-900/50 border border-slate-800/80 shadow-2xl space-y-6 scrollbar-thin scrollbar-thumb-slate-700">
          {session.messages.map((msg) => {
            ...
            return (
              ...
              >
                {isAI && (
                  <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-indigo-600 to-cyan-500 p-0.5 shrink-0 shadow-lg shadow-indigo-500/20">
                ...
                <div className={`max-w-3xl space-y-2 ${isAI ? 'text-left' : 'text-right'}`}>
                  {/* Message Metadata */}
                  <div className={`flex items-center gap-2 text-[11px] text-slate-400 ${isAI ? '' : 'justify-end'}`}>
                    <span className="font-semibold text-slate-200">{isAI ? 'AI Technical Interviewer' : 'Candidate (You)'}</span>
                    <span>•</span>
                    <span>{msg.timestamp}</span>
                    {msg.difficulty && (
                      <span className="px-2 py-0.5 rounded bg-indigo-500/10 border border-indigo-500/30 text-indigo-300 font-mono text-[10px]">
                  ...
                  <div
                    className={`p-5 rounded-2xl text-sm leading-relaxed ${
                      isAI
                        ...
                        : 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-tr-none shadow-xl'
                    }`}
                  >
                    ...
                    {msg.code_snippet && (
                      <div className="mt-4 p-4 rounded-xl bg-slate-950/90 border border-slate-800 text-left font-mono text-xs overflow-x-auto text-cyan-300">
                        <div className="flex items-center justify-between text-[10px] text-slate-500 uppercase tracking-wider mb-2 pb-2 border-b border-slate-800">
                ...
                {!isAI && (
                  <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-purple-500 to-indigo-600 p-0.5 shrink-0 shadow-lg shadow-purple-500/20">
                    <div className="w-full h-full bg-slate-900 rounded-[14px] flex items-center justify-center text-purple-300">
        ...
        <div className="p-4 sm:p-6 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl space-y-4">
          ...
          {activeTab === 'text' ? (
            <div className="space-y-3">
              <textarea
                ...
                rows={4}
                className="w-full p-4 rounded-2xl bg-slate-950/80 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 text-sm font-normal leading-relaxed transition-colors resize-none"
              />
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {[
                  ...
                  <button
                    ...
                    className="text-[11px] px-2.5 py-1 rounded-lg bg-slate-800/60 hover:bg-slate-800 border border-slate-700/60 text-slate-400 hover:text-indigo-300 transition-colors"
                  >
                    + {chip.slice(0, 30)}...
                  </button>
                ))}
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <button
              ...
              className="px-6 py-3 rounded-xl bg-gradient-to-r from-indigo-500 via-blue-600 to-cyan-500 hover:from-indigo-600 hover:to-cyan-600 text-white font-bold text-sm shadow-xl shadow-indigo-500/20 flex items-center gap-2.5 disabled:opacity-50 transition-all hover:scale-[1.02] cursor-pointer"
            >
              {submitting ? (
                <>
                  <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
          ...
          >
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-slate-400">
                Track: <span className="text-indigo-300 font-semibold">{summary.track_title}</span> • Role: <span className="text-cyan-300 font-semibold">{summary.target_role}</span>
              </p>
            </div>

            {/* Scorecard Hero Metrics */}
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 p-5 rounded-2xl bg-slate-800/60 border border-slate-700/80 text-center">
              <div>
                <div className="text-[11px] font-mono text-slate-400 uppercase">Overall Score</div>
                <div className="text-3xl font-black text-emerald-400 mt-1">{summary.overall_score}%</div>
              </div>
              <div>
                <div className="text-[11px] font-mono text-slate-400 uppercase">Tech Accuracy</div>
                <div className="text-3xl font-black text-indigo-400 mt-1">{summary.technical_accuracy}%</div>
              </div>
              <div>
                <div className="text-[11px] font-mono text-slate-400 uppercase">Problem Solving</div>
                <div className="text-3xl font-black text-cyan-400 mt-1">{summary.problem_solving}%</div>
              </div>
              <div>
                <div className="text-[11px] font-mono text-slate-400 uppercase">Communication</div>
                <div className="text-3xl font-black text-purple-400 mt-1">{summary.communication}%</div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-indigo-950/30 border border-indigo-500/20 text-xs text-slate-300 leading-relaxed">
              <span className="font-semibold text-indigo-300 block mb-1">AI Evaluator Summary:</span>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                onClick={() => navigate('/dashboard')}
                className="w-full py-3.5 rounded-xl bg-gradient-to-r from-indigo-500 to-cyan-500 text-white font-bold text-sm shadow-xl flex items-center justify-center gap-2 cursor-pointer hover:scale-[1.01] transition-transform"
              >
